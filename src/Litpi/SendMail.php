    ...
    public function send()
    {
        ...
        if ($this->registry->setting['mail']['usingAmazonses']) {
            ...
            try {
                ...
                $result = false;
                //\Model\SocketDebug::d($e->getMessage(), 1);
            ...
            if (!isset($this->registry->setting['smtp']['port'])) {
                $smtpport = 465;
            } else {
                $smtpport = $this->registry->setting['smtp']['port'];
            }

            if (!isset($this->registry->setting['smtp']['ssl'])) {
                $smtpssl = 'ssl';
            } else {
                $smtpssl = $this->registry->setting['smtp']['ssl'];
            }
            ...
            if ($this->usingSMTP) {
                ...
                $this->mailer->SMTPSecure = $smtpssl;
