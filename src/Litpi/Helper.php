...
class Helper
{
    ...
    */
    public static function strtotimedmy($datestring = '01/01/1970', $timestring = '00:00:01')
    {
    ...
    */
    ...
    public static function curPageURL()
    {
        ...
        $pageURL = preg_replace('/language=[a-z]{2}/', '', $pageURL);
    ...
    */
    ...
    public static function taglistStringToArray($taglistString)
    {
        $taglist = array();
        $taglistTmp = explode(',', $taglistString);
        foreach ($taglistTmp as $tag) {
            $tag = Helper::plaintext(trim($tag));
    ...
    public static function taglistArrayToString($taglistArray)
    {
        return implode(', ', $taglistArray);
    }
    ...
    public static function removeEmptyParagraphTinymce($input)
    {
        return trim(preg_replace('/<p>\p{Z}*<\/p>/u', '', $input));
    }

    public static function getMonthToDate()
    {
    ...
    public static function formatSecondToTimeString($second)
    {
        $timestring = '';

        //get day
        if ($second >= 86400) {
            $timestring .= (int)($second / 86400) . ' ngày ';
            $second -= (int)($second / 86400) * 86400;
        }

        //get hour
        if ($second > 3600) {
            $timestring .= (int)($second / 3600) . ' giờ ';
            $second -= (int)($second / 3600) * 3600;
        }

        //we show minutes if empty string
        if ($timestring == '') {
            $timestring .= (int)($second / 60) . ' phút ';
        }
